...
for (const width of widths) for (const route of routes) {
  ...
  try {
    ...
    if(route==='index.html'){
      record.homeGeometry=await page.eval(`(()=>{const stats=document.querySelector('#stats'),chooser=document.querySelector('.home-track-compare'),title=document.querySelector('#prize-heading');return {statsBeforeChoices:!!(stats.compareDocumentPosition(chooser)&Node.DOCUMENT_POSITION_FOLLOWING),prizeTitle:title.textContent.trim(),prizeFont:parseFloat(getComputedStyle(title).fontSize),cards:[...chooser.querySelectorAll('a')].map(a=>{const t=a.querySelector('strong').getBoundingClientRect(),d=a.querySelector('small').getBoundingClientRect(),r=a.getBoundingClientRect();return {gap:d.top-t.bottom,inside:d.left>=r.left&&d.right<=r.right}})}})()`);
      if(!record.homeGeometry.statsBeforeChoices||record.homeGeometry.prizeTitle.includes('$18,000')||record.homeGeometry.prizeFont>64||record.homeGeometry.cards.some(c=>c.gap<3||!c.inside))report.findings.push({surface:label,type:'home-hierarchy',impact:'serious',measurement:record.homeGeometry});
    }
